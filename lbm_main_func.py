...
data1 = []
for z in range(1, 31):
    file_name = f"C:/Users/YYBG-WANG/Desktop/科研/4.双聚类(Biclustering)python包/对照数据/lbm_sample/data1/data1_{z}.csv"
    df = pd.read_csv(file_name)
    data1.append(df.values)
data1 = np.array(data1)
data1 = np.transpose(data1, (1, 2, 0))
from simulation_data import lbm_sim_data
lbm_simdata1 = lbm_sim_data()
lbm_simdata1['data1'] = data1

data1 = []
for z in range(1, 31):
    file_name = f"C:/Users/YYBG-WANG/Desktop/科研/4.双聚类(Biclustering)python包/对照数据/lbm_sample/data2/data1_{z}.csv"
    df = pd.read_csv(file_name)
    data1.append(df.values)
data1 = np.array(data1)
data1 = np.transpose(data1, (1, 2, 0))
from simulation_data import lbm_sim_data
lbm_simdata2 = lbm_sim_data(bivariate = True)
lbm_simdata2['data1'] = data1

data2 = []
for z in range(1, 31):
    file_name = f"C:/Users/YYBG-WANG/Desktop/科研/4.双聚类(Biclustering)python包/对照数据/lbm_sample/data2/data2_{z}.csv"
    df = pd.read_csv(file_name)
    data2.append(df.values)
data2 = np.array(data2)
data2 = np.transpose(data2, (1, 2, 0))
from simulation_data import lbm_sim_data
lbm_simdata2['data2'] = data2
'''
