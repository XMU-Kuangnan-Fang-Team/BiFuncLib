...
# cc
from simulation_data import cc_sim_data
from cc_bifunc import cc_bifunc
fun_mat = cc_sim_data()
cc_result = cc_bifunc(data = fun_mat, delta = 10, alpha = 1, beta = 0, const_alpha = True, shift_alignment = True)
FDPlot(cc_result).cc_fdplot(fun_mat, only_mean = False, aligned = True, warping = True)


# lbm
from simulation_data import lbm_sim_data
from lbm_bifunc import lbm_bifunc
lbm_simdata1 = lbm_sim_data(n = 50, p = 50, t = 15)
lbm_simdata2 = lbm_sim_data(n = 50, p = 50, t = 15, bivariate = True)
data1 = [lbm_simdata2['data1'],lbm_simdata2['data2']]
data2 = lbm_simdata1['data']
lbm_res = lbm_bifunc(data1, K=4, L=3)
lbm_res_grid = lbm_bifunc(data2, K=[2,3,4], L=[2,3])
FDPlot(lbm_res).lbm_fdplot('proportions')
FDPlot(lbm_res_grid).lbm_fdplot('blocks')
FDPlot(lbm_res_grid).lbm_fdplot('means')


# local
from local_bifunc import local_bifunc
...
from sas_bifunc import sas_bifunc, sas_bifunc_cv
...
from sparse_bifunc import sparse_bifunc
K = 2
paramC = 2
n = 10
...
sparse_res = sparse_bifunc(data, x, K)
sparse_res_2 = sparse_bifunc(data, x, K, true_clus = part_vera)
FDPlot(sparse_res).sparse_fdplot(x, data)
FDPlot(sparse_res_2).sparse_fdplot(x, data)
'''
