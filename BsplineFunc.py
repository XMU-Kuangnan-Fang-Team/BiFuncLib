...
nbasis = 15
x = np.array(location)
basis = create_bspline_basis(rangeval=[min(x), max(x)], nbasis=nbasis, norder=4)
bspline = BsplineFunc(basis)
smoothed_functions = []
for i in range(1000):
    y_noisy = np.sin(2 * np.pi * x) + np.random.normal(scale=0.1, size=n_obs)
    smoothed = bspline.smooth_basis(x, y_noisy.reshape(-1, 1))
    smoothed_functions.append(smoothed)
'''