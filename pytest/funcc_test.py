...
def test_cc_bifunc_cv_smoke():
    ...
    res_cv = cc_bifunc_cv(fun_mat, delta_list=delta_list, alpha=1, beta=0, const_alpha=True, const_beta=True)
    assert isinstance(res_cv, pd.DataFrame)

def test_cc_bifunc_no_shift():
    fun_mat = cc_sim_data()
    res = cc_bifunc(fun_mat, delta=10, template_type='medoid', alpha=0, beta=0, const_alpha=True, shift_alignment=False)
...
def test_max_iter_align_non_positive():
    with pytest.raises(ValueError, match="max.iter.align must be.*greater than 0"):
