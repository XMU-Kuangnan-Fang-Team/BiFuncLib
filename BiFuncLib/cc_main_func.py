...
def template_evaluation(fun_mat, a, b, const_a, const_b):
    ...
    for j in range(n):
        row_data = fun_mat[j, :, :]
        alpha_fun[j] = np.nansum(row_data, axis=0) / np.sum(
            not_null[j, :], axis=0
        )
    alpha_fun = alpha_fun - fun_mean
    if const_a:
        alpha_fun = np.tile(
            np.nanmean(alpha_fun, axis=1, keepdims=True), (1, p)
        )
    beta_fun = np.zeros((m, p))
    for j in range(m):
        col_data = fun_mat[:, j, :]
        beta_fun[j] = np.nansum(col_data, axis=0) / np.sum(
            not_null[:, j], axis=0
        )
            ...
            for i in range(n):
                for j in range(m):
                    if not_null[i, j]:
                        ...
                        fun_mat_align[i, j, :] = interp1d(
                            new_x, fun_mat[i, j, :], fill_value="extrapolate"
                        )(x_out)
                    if not not_null[i, j]:
                        dist_mat[i, j] = np.nan
...
def warping_function_add(
    ...
    while iter_count < max_iter and not conv:
        ...
        dist_mat_old = dist_mat.copy()
        if template_type == "mean":
            new_fun = template_evaluation_add(
                fun_mat_align, logr, logc, a, b, const_a, const_b
            )
        elif template_type == "medoid":
            new_fun = medoid_evaluation_add(
                fun_mat_align, logr, logc, a, b, const_a, const_b
            )
        if shift_alignment:
            for i in range(n):
...
def evaluate_mat_dist_add(
    ...
    if shift_alignment:
        ...
        if template_type == "mean":
            new_fun = template_evaluation_add(
                fun_mat, logr, logc, a, b, const_a, const_b
            )
        elif template_type == "medoid":
            new_fun = medoid_evaluation_add(
                fun_mat, logr, logc, a, b, const_a, const_b
...
def cc1_fun(
    ...
    while score_while > delta:
        logr[logr] = np.sum(
            np.isnan(
                fun_mat[np.ix_(logr, logc, [True] * fun_mat.shape[2])][:, :, 0]
            ),
            axis=1,
        ) != np.sum(logc)
        logc[logc] = np.sum(
        ...
        if only_one == "False" and not (np.sum(logr) > 1 and np.sum(logc) > 1):
            break
        if only_one == "True" and not (
            (np.sum(logr) >= 1 and np.sum(logc) > 1)
            or (np.sum(logr) > 1 and np.sum(logc) >= 1)
        ):
            break
        if only_one == "True_alpha" and not (
            np.sum(logr) >= 1 and np.sum(logc) > 1
        ):
            break
        if only_one == "True_beta" and not (
            np.sum(logr) > 1 and np.sum(logc) >= 1
        ):
            break
        sub_mat = fun_mat[np.ix_(logr, logc, [True] * fun_mat.shape[2])]
    ...
    if only_one == "False":
        ...
        if (np.sum(logr) >= 1 and np.sum(logc) > 1) or (
            np.sum(logr) > 1 and np.sum(logc) >= 1
        ):
            ret = [logr, logc]
...
def cc3_fun(
    ...
    while br > 0:
        ...
        br1 = np.sum(logc)
        br2 = np.sum(logr)
        dist_mat_add = evaluate_mat_dist_add(
            fun_mat,
            logr,
            logc,
            template_type,
...
def warping_function_plot(
    ...
    def warping_shift(coeff):
        ...
        b_mask = ~np.isnan(template_t)
        data_t = interp1d(
            st, fun_mat[i, j, :], bounds_error=False, fill_value="extrapolate"
        )(x_out)
        a_mask = ~np.isnan(data_t)
        sel = a_mask & b_mask
        data_t = data_t[sel]
    ...
    align_mat = np.zeros((n, m))
    count_null = np.apply_along_axis(
        lambda arr: np.sum(np.isnan(arr)), 2, fun_mat
    )
    not_null = count_null < p
    ...
    count_null = np.apply_along_axis(
        lambda x: np.sum(np.isnan(x)), axis=2, arr=fun_mat
    )
    not_null = count_null < p
    fun_mat_align = fun_mat.copy()
    iter_count = 0
