...
def pf_sim_data(n, T, nknots, order, seed=123):
    ...
    for i in range(n):
        ...
        data = {
            "id": np.repeat(np.arange(q), id_matrix[i, :]),
    ...
    for i, sample in enumerate(sample_list):
        ...
        for _, group in sample.groupby("id"):
            basis = bspline_mat(
                np.array(group["time"]), auxfunc_1.knots_eq(), norder=order
            )
    ...
    for i in range(subgroup_sam):
        for j in range(subgroup_fea):
            ...
            mat = np.arange(num_elem).reshape(
                cluster_struc_sam[i], cluster_struc_fea[j]
            )
            ...
            for l in range(len(id1)):
                ...
                j1 = id_loac[l, 1]
                id2 = np.sort(
                    np.random.choice(T, int((1 - miss_meas) * T), replace=False)
                )
                times[i1 + cluster_gap_sam[i]][j1 + cluster_gap_fea[j]] = t[id2]
                basis_new = bspline_mat(
                    t[id2], auxfunc_1.knots_eq(), norder=order
                )
                spline_list[i1 + cluster_gap_sam[i]][
                    j1 + cluster_gap_fea[j]
                ] = basis_new
                Y_list[i1 + cluster_gap_sam[i]][j1 + cluster_gap_fea[j]] = (
                    Y_list[i1 + cluster_gap_sam[i]][j1 + cluster_gap_fea[j]][
                        id2
                    ]
                )
    ...
    for i in range(len(censored_none_list)):
        pivot_df = censored_none_list[i].pivot_table(
            index="time", columns="id", values="y"
        )
        ...
        censored_sample_matrix = pd.concat(
            [censored_sample_matrix, pivot_df], axis=0
        )
...
def cc_sim_data():
    ...
    data_dict = {
        group: matrix.drop(columns=["Matrix"]).values.tolist()
        for group, matrix in ccdata.groupby("Matrix")
    }
    sorted_keys = sorted(
        [int(key.replace("Matrix", "")) - 1 for key in data_dict.keys()]
    )
...
def lbm_sim_data(n=100, p=100, t=30, bivariate=False, noise=None, seed=111):
    ...
    Z = np.concatenate(
        [np.full(cnt, i + 1) for i, cnt in enumerate(counts_rows)]
    )
    ...
    W = np.concatenate(
        [np.full(cnt, i + 1) for i, cnt in enumerate(counts_cols)]
    )
    if bivariate:
    ...
    for k in range(K):
        for l in range(L):
            ...
            block_data = signal_block.reshape(
                (n_rows, n_cols, long_), order="F"
            )
            ...
            if bivariate:
                ...
                block_data2 = signal_block2.reshape(
                    (n_rows, n_cols, long_), order="F"
                )
...
def sas_sim_data(
    ...
    if scenario == 0:
        part = nbasis // 6
        mean1 = np.concatenate(
            (np.repeat(1.5, part), np.repeat(0, nbasis - part))
        )
        mean2 = np.concatenate(
            (np.repeat(-1.5, part), np.repeat(0, nbasis - part))
        )
    ...
    if len(mean_list) == 1:
        X_coef = np.transpose(
            multivariate_normal(mean_list[0], cov_mat).rvs(size=n_i)
        )
    else:
        X_coef = np.transpose(
            multivariate_normal(mean_list[0], cov_mat).rvs(size=n_i)
        ...
        for ii in range(1, len(mean_list)):
            X_coef = np.concatenate(
                (
                    X_coef,
                    np.transpose(
                        multivariate_normal(mean_list[ii], cov_mat).rvs(
                            size=n_i
                        )
                    ),
                ),
...
def sparse_sim_data(n, x, paramC, plot=False):
    ...
    clusters = np.hstack((np.ones(n, dtype=int), np.full(n, 2, dtype=int)))
    if plot:
