...
def sas_bifunc(
    ...
    if trace:
        print(
            f"Iteration 0: Sigma = {sigma_new} loglk = {lk_temp[0]} ploglk = {lk_temp[1]}"
        )
    lk_new = lk_temp[1]
    ...
    while abs(lk_old - lk_new) > tol and (ind <= maxit):
        ...
        if trace:
            print(
                f"Iteration {ind}: Sigma = {sigma_new} loglk = {lk_i[0]} ploglk = {lk_i[1]}"
            )
        if plot:
            basis = create_bspline_basis((grid[0], grid[-1]), nbasis=q)
            mean_fd = fd(parameters["mu"].T, basis)
...
def sas_bifunc_cv(
    ...
    if np.ndim(X) == 2:
        ...
        if timeindex is None:
            raise ValueError(
                "For irregularly sampled functional data, timeindex must be provided"
            )
        if curve is None:
            raise ValueError(
                "For irregularly sampled functional data, curve must be provided"
            )
    ...
    if X_test is None:
        ...
        def parr_fun(ii):
            ...
            for lll in range(K_fold):
                train_indices = np.concatenate(
                    [split_vec[j] for j in range(K_fold) if j != lll]
                )
                ...
                if np.ndim(X) == 2:
                    ...
                    X_fold = np.concatenate(
                        [
                            X[np.where(np.array(curve) == idx)]
                            for idx in train_indices
                        ]
                    )
                    timeindex_fold = np.concatenate(
                        [
                            np.array(timeindex)[
                                np.where(np.array(curve) == idx)
                            ]
                            for idx in train_indices
                        ]
                    )
                    curve_fold = np.concatenate(
                        [
                            np.full(np.sum(np.array(curve) == idx), idx)
                            for idx in train_indices
                        ]
                    )
                else:
                ...
                l_val = loglik(
                    parameters=mod["mod"]["parameters"],
                    X=X_i,
                    timeindex=timeindex_i,
                    curve=curve_i,
                    grid=grid_i,
                    vars_=mod["mod"]["vars"],
                    FullS=mod["mod"]["FullS"],
                    W=mod["mod"]["W"],
                    AW_vec=mod["mod"]["AW_vec"],
                    P_tot=mod["mod"]["P_tot"],
                )
                l_list.append(l_val)
        ...
        def parr_fun(ii):
            ...
            l_val = loglik(
                ...
                AW_vec=mod["mod"]["AW_vec"],
                P_tot=mod["mod"]["P_tot"],
                lambda_s=mod["mod"].get("lambda_s"),
                lambda_l=mod["mod"].get("lambda_l"),
            )
