...
def calculate_gcv(
    oridata, oritimes, lambda1_seq, rangeval=(0, 1), nknots=30, order=4, nu=2
):
    ...
    for l in range(nlambda1):
        ...
        for i in range(n):
            ni = len(oridata[i])
            Hi = Z_lst[i] @ np.linalg.solve(
                Z_lst[i].T @ Z_lst[i] + lambda1_seq[l] * D, Z_lst[i].T
            )
...
def generate_inicoef(
    ...
    for i in range(n):
        Z_i = Z_lst[i]
        Beta_ini[:, i] = (
            np.linalg.solve(Z_i.T @ Z_i + lambda1 * D, (Z_i.T @ oridata[i]).T)
        ).flatten()
    ...
    for j in range(p):
        if l <= j <= l + r:
            if xl_norm <= lamda * varsigma:
                ...
                z[j] = (
                    (tau / (tau - varsigma))
                    * max(1 - varsigma * lamda / xl_norm, 0)
                    * x[j]
                )
            else:
                z[j] = x[j]
...
def update_Beta(
    ...
    for i in range(n):
        ...
        while res_inner > eps_inner and iter_inner <= max_iter_inner:
            V_middle[:, i] = U_middle[:, i] - varsigma1 * (
                ZTZ_lst[i].dot(U_middle[:, i])
                - ZTY_lst[i]
                + lambda1 * D.dot(U_middle[:, i])
            )
            center_index_opt = assign_center_label(V_middle[:, i], Alpha)
            Beta_new[:, i] = (
                proximal_MCP(
                    V_middle[:, i] - Alpha[:, center_index_opt],
                    varsigma1,
                    tau,
                    lambda2,
                )
                + Alpha[:, center_index_opt]
            )
            eta_new = (1 + np.sqrt(1 + 4 * eta_old**2)) / 2
            U_middle[:, i] = Beta_new[:, i] + (eta_old - 1) / eta_new * (
                Beta_new[:, i] - Beta_old[:, i]
            )
...
def update_Alpha(
    ...
    while res_inner > eps_inner and iter_inner <= max_iter_inner:
        v_middle = u_middle - varsigma2 * kappa / n * (
            OmegaTOmega @ u_middle - OmegaTpsi + OmegaTgamma
        )
        ...
        u_middle = (
            Alpha_new.T.flatten()
            + (eta_old - 1) / eta_new * (Alpha_new - Alpha_old).T.flatten()
        )
...
def update_Psi(
    ...
    for s in range(S):
        ...
        eps_inner = 0.01 / iter_outer
        while res_inner > eps_inner and iter_inner <= max_iter_inner:
            V_middle[:, s] = U_middle[:, s] - varsigma3 * kappa * (
                U_middle[:, s]
                - (
                    Alpha[:, index[s, 0]]
                    - Alpha[:, index[s, 1]]
            ...
            for l in range(L):
                Psi_new[:, s] += proximal_average_MCP(
                    V_middle[:, s], l, r, varsigma3, tau, lambda3
                )
            Psi_new[:, s] /= L
            for l in range(L):
                Psi_new[l : (l + r + 1), s] = proximal_MCP(
                    Psi_new[l : (l + r + 1), s], varsigma3, tau, lambda3
                )
            eta_new = (1 + np.sqrt(1 + 4 * eta_old**2)) / 2
            U_middle[:, s] = Psi_new[:, s] + (eta_old - 1) / eta_new * (
                Psi_new[:, s] - Psi_old_[:, s]
            )
...
def update_Gamma(Gamma_old, Alpha, Psi, index, kappa):
    ...
    for s in range(S):
        ...
        Gamma[:, s] = Gamma_old[:, s] + kappa * (
            Alpha[:, i] - Alpha[:, j] - Psi[:, s]
        )
    return Gamma
...
def local_admm(
    ...
    while res_outer > eps_outer and iters <= max_iter:
        ...
        for s in range(S):
            i = index[s, 0]
            j = index[s, 1]
            res_outer_vec[s] = np.sum(
                np.square(Alpha_new[:, i] - Alpha_new[:, j] - Psi_new[:, s])
            )
        ...
        def convert_to_class_list(clusters):
            ...
            result = [
                element_to_class[i] for i in sorted(element_to_class.keys())
            ]
            return result

        for l in range(L):
            Ad_final = AuxFunc(
                n=K0, V=Psi_new[l : (l + r + 1), :]
            ).create_adjacency(plot=False)
    ...
    for i in range(n):
        ...
        Beta_new[:, i] = (
            proximal_MCP(
                Beta_new[:, i] - Alpha_new[:, center_index_opt],
                varsigma1,
                tau,
                lambda2,
            )
            + Alpha_new[:, center_index_opt]
        )
    ...
    for k in range(cls_num):
        Alpha_trans[:, k] = np.mean(
            Beta_trans[:, group == unique_values[k]], axis=1
        )
    ...
    for l1 in range(nlambda2):
        for l2 in range(nlambda3):
            ...
            funlc_result = local_admm(
                ...
                rep_num=rep_num,
                lambda1=lambda1,
                lambda2=lambda2_seq[l1],
                lambda3=lambda3_seq[l2],
                kappa=kappa,
                eps_outer=eps_outer,
                max_iter=max_iter,
            )
            ...
            for i in range(n):
                error_vec[i] = np.sum(
                    np.square(oridata[i] - np.dot(Z_lst[i], Beta[:, i]))
                )
                Hi = np.dot(
                    Z_lst[i],
                    np.linalg.solve(
                        np.dot(Z_lst[i].T, Z_lst[i]) + lambda1 * D, Z_lst[i].T
                    ),
                )
