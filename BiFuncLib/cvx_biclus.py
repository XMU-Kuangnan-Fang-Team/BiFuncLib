...
def cvx_biclus_valid(
    ...
    if plot_clus == True:
        if smooth == True:
            sns.clustermap(
                M, cmap="Blues", yticklabels=False, xticklabels=False
            )
            plt.show()
        elif smooth == False:
            sns.clustermap(
                data, cmap="Blues", yticklabels=False, xticklabels=False
            )
            plt.show()
    return sol
...
def cvx_biclus_missing(
    ...
    if plot_clus == True:
        sns.clustermap(
            sol["U"], cmap="Blues", yticklabels=False, xticklabels=False
        )
        plt.show()
    return sol
