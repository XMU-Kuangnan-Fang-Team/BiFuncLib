...
def bcheatmap(
    ...
    fig, (ax, cax) = plt.subplots(
        1, 2, gridspec_kw={"width_ratios": [8.5, 1.5]}, figsize=(10, 8)
    )
    if number == 1:
        ...
        roworder = np.concatenate(
            [np.where(rowmat[:, 0])[0], np.where(~rowmat[:, 0])[0]]
        )
        colorder = np.concatenate(
            [np.where(colmat[:, 0])[0], np.where(~colmat[:, 0])[0]]
        )
        ...
        if axisC:
            ...
            if hasattr(res, "NumberxCol") and hasattr(
                res.NumberxCol, "columns"
            ):
                ax.set_xticklabels(
                    res.NumberxCol.columns[colorder],
                    rotation=90,
                    fontsize=cexC * 10,
                )
            else:
        ...
        if axisR:
            ...
            if hasattr(res, "RowxNumber") and hasattr(res.RowxNumber, "index"):
                ax.set_yticklabels(
                    res.RowxNumber.index[roworder], fontsize=cexR * 10
                )
            else:
        ...
        if number > 2:
            for i in range(number - 2):
                innext = np.intersect1d(
                    np.where(rowmat[:, i])[0], np.where(rowmat[:, i + 1])[0]
                )
                nooverlap = np.where(
                ...
                for l in range(1, number - i):
                    temp = np.intersect1d(
                        np.where(rowmat[:, i])[0], np.where(rowmat[:, i + l])[0]
                    )
        ...
        if number > 2:
            for i in range(number - 2):
                innext = np.intersect1d(
                    np.where(colmat[:, i])[0], np.where(colmat[:, i + 1])[0]
                )
                nooverlap = np.where(
                ...
                for l in range(1, number - i):
                    temp = np.intersect1d(
                        np.where(colmat[:, i])[0], np.where(colmat[:, i + l])[0]
                    )
                    temp = np.setdiff1d(temp, innext)
                    colorder = np.unique(
