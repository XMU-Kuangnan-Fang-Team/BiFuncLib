...
def empty_class_check(W, Pw=None, display=False):
    ...
    while np.min(np.sum(W, axis=0)) < 1 and nb < 10:
        ...
        if display:
            print(
                "Warning: Class is becoming empty! A regularization has been done."
            )
        nb += 1
    return W
...
def mypca_fd(fdobj_, center=True):
    ...
    if isinstance(fdobj, list):
        ...
        mat_interm = (
            coef @ cholesky(W_tot) / np.sqrt(fdobj[0]["coefs"].shape[1] - 1)
        )
...
def estep_Z_cost(xx, alpha, beta, mu, a, b, d, Q, k, l, Wmat):
    ...
    Pa = (
        (xx - np.ones((xx.shape[0], 1)) @ mu[k, l, :].reshape(1, -1))
        @ Qkl
        @ Qkl.T
    )
    Pb = Pa + (np.ones((xx.shape[0], 1)) @ mu[k, l, :].reshape(1, -1)) - xx
    A = (
        1 / a[k, l] * np.sum(np.square(Pa), axis=1)
        + 1 / b[k, l] * np.sum(np.square(Pb), axis=1)
        + d[k, l] * np.log(a[k, l])
        + (nbasis - d[k, l]) * np.log(b[k, l])
        - 2 * np.log(beta[l])
    )
    return A.T
...
def estep_Z(X, alpha, beta, mu, a, b, d, Q, W, Wmat):
    ...
    for k in range(K):
        ...
        for l in range(L):
            Xl = X[:, W[:, l] == 1, :]
            if np.sum(W[:, l]) == 1:
                A[:, W[:, l] == 1] = estep_Z_cost(
                    Xl, alpha, beta, mu, a, b, d, Q, k, l, Wmat
                )
            else:
                A[:, W[:, l] == 1] = np.column_stack(
                    [
                        estep_Z_cost(
                            Xl[:, j, :], alpha, beta, mu, a, b, d, Q, k, l, Wmat
                        ).T
    ...
    P = np.array(
        [1 / np.sum(np.exp(row - row[k])) for row in P for k in range(K)]
    ).reshape(P.shape)
    return P
...
def estep_W(X, alpha, beta, mu, a, b, d, Q, Z, Wmat):
    ...
    for l in range(L):
        ...
        for k in range(K):
            Xk = X[Z[:, k] == 1, :, :]
            if np.sum(Z[:, k]) == 1:
                A[Z[:, k] == 1, :] = estep_W_cost(
                    Xk, alpha, beta, mu, a, b, d, Q, k, l, Wmat
                )
            else:
                A[Z[:, k] == 1, :] = np.column_stack(
                    [
                        estep_W_cost(
                            Xk[:, j, :], alpha, beta, mu, a, b, d, Q, k, l, Wmat
                        ).T
    ...
    P = np.array(
        [1 / np.sum(np.exp(row - row[l])) for row in P for l in range(L)]
    ).reshape(P.shape)
    return P
...
def compute_complete_likelihood(X, alpha, beta, mu, a, b, d, Q, Z, W, Wmat):
    ...
    for l in range(L):
        for k in range(K):
            ...
            lik = lik - 0.5 * np.sum(
                estep_lik_cost(Xkl, alpha, beta, mu, a, b, d, Q, k, l, Wmat)
            )
    return lik
...
def lbm_main_func(
    ...
    if not isinstance(X, list):
        Xcol = np.column_stack(
            [X[:, :, t].flatten("F") for t in range(X.shape[2])]
        )
        ...
        for i in range(len(X)):
            X_sel = X[i]
            Xcol = np.column_stack(
                [X_sel[:, :, t].flatten("F") for t in range(X_sel.shape[2])]
            )
            obj_s = BsplineFunc(basis).smooth_basis(
                np.arange(1, T + 1), Xcol.T
            )["fd"]
            Coefs = obj_s["coefs"].T
    ...
    if init == "kmeans":
        if not isinstance(X, list):
            z1 = np.random.choice(
                np.arange(N), size=int(0.3 * N), replace=False
            )
            z2 = np.random.choice(
                np.arange(p), size=int(0.3 * p), replace=False
            )
            obj_s = BsplineFunc(basis).smooth_basis(
                np.arange(1, T + 1), Xcol.T
            )["fd"]
    ...
    if init == "funFEM":
        ...
        if not isinstance(X, list):
            fd = BsplineFunc(basisfunFEM).smooth_basis(
                np.arange(1, p * T + 1), np.vstack(np.transpose(X, (2, 1, 0)))
            )["fd"]
            while True:
                try:
                    out_funFEM = fem_bifunc(
                        fd, np.atleast_1d(K).tolist(), maxit=10
                    )
                    break
                except ValueError:
                    print("Rerun")
            Z = np.apply_along_axis(
                lambda x: (x >= np.max(x)).astype(int), 1, out_funFEM["P"]
            )
        else:
            fd = BsplineFunc(basisfunFEM).smooth_basis(
                np.arange(1, p * T + 1),
                np.vstack(np.transpose(X[0], (2, 1, 0))),
            )["fd"]
            while True:
                try:
                    out_funFEM = fem_bifunc(
    ...
    if init == "funFEM":
        ...
        if not isinstance(X, list):
            fd = BsplineFunc(basisfunFEM).smooth_basis(
                np.arange(1, N * T + 1), np.vstack(np.transpose(X, (2, 0, 1)))
            )["fd"]
            while True:
                try:
                    out_funFEM = fem_bifunc(
                        fd, np.atleast_1d(L).tolist(), maxit=10
                    )
                    break
                except ValueError:
                    continue
            ...
            fd = BsplineFunc(basisfunFEM).smooth_basis(
                np.arange(1, N * T + 1),
                np.vstack(np.transpose(X[0], (2, 0, 1))),
            )["fd"]
            while True:
                try:
                    out_funFEM = fem_bifunc(
    ...
    for l in range(L):
        mu[:, l, :] = (
            np.tile(base_coefs, K)
            + np.tile(
                np.random.normal(0, 0.01, K * nbasis), base_coefs.size // nbasis
            )
        ).reshape(K, -1)
    ...
    for it in range(maxit):
        ...
        for k in range(K):
            for l in range(L):
                ...
                b[k, l] = np.mean(
                    dc["values"][int(d[k, l]) : len(dc["values"])]
                )
        ...
        for r in range(gibbs_it):
            ...
            if np.min(np.sum(W, axis=0)) < 1 or np.min(np.sum(Z, axis=0)) < 1:
                raise Exception(
                    "One class is empty, re-run the algorithm or choose an other number of clusters"
                )
        lik[it] = compute_complete_likelihood(
            C_tot, alpha, beta, mu, a, b, d, Q, Z, W, Wmat
        )
        Alphas[it, :] = alpha
        Betas[it, :] = beta
