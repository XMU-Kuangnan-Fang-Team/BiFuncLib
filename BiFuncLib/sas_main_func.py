...
def loglik(
    ...
    if data is None:
        ...
        if X.ndim == 2:
            ...
            if grid is None:
                raise ValueError(
                    "For irregularly sampled functional data grid must be provided"
                )
            if timeindex is None:
                raise ValueError(
                    "For irregularly sampled functional timeindex grid must be provided"
                )
            if curve is None:
                raise ValueError(
                    "For irregularly sampled functional timeindex curve must be provided"
                )
            data = {
                "x": np.atleast_2d(X),
                "timeindex": timeindex,
...
def sasfclust_init(
    data,
    pert=0,
    grid=list(np.linspace(0.01, 1, 100)),
    q=5,
    G=1,
    der=0,
    gamma_ada=1,
    ...
    if G != 1:
        ...
        for ii in range(G - 1):
            ...
            ind = [
                min((((G - 1) ** 2 + (G - 1)) // 2, ind[0])),
                min((((G - 1) ** 2 + (G - 1)) // 2, ind[1])),
            ]

        P_tot = np.zeros((q * ((G - 1) ** 2 + (G - 1)) // 2, G * q))
        for ii in range(G):
            P_tot[:, ii * q : (ii + 1) * q] = np.kron(
                P[:, ii].reshape(-1, 1), np.eye(q)
            )
    ...
    breaks = basis["params"]
    ext_break = np.concatenate(
        [np.repeat(grid[0], order), breaks, np.repeat(grid[-1], order)]
    )
    weights_vec = np.repeat(
        (ext_break[order:] - ext_break[:-order]) / order,
        ((G - 1) ** 2 + (G - 1)) / 2,
    )
    ...
    if pert is not None:
        ...
        for i in range(N):
            ...
            points[i, :] = (
                np.linalg.inv(Si.T @ Si + pert * np.eye(q)) @ Si.T @ xi
            )
    else:
        d = np.array(
            [
                len(data["curve"][data["curve"] == np.unique(data["curve"])[i]])
                for i in range(N)
            ]
        )
        e = [
            data["timeindex"][data["curve"] == np.unique(data["curve"])[i]]
            for i in range(N)
        ]
        if len(np.unique(d)) == 1 and len([np.unique(e)]) == 1:
            ...
            grid_i = list(
                np.array(grid)[
                    data["timeindex"][
                        data["curve"] == np.unique(data["curve"])[0]
                    ]
                    - 1
                ]
            )
            ...
            for i in range(len(loglam)):
                fdPari = fdpar(
                    fdobj=basis_start, Lfdobj=2, lambda_=10 ** loglam[i]
                )
                Sm_i = BsplineFunc(fdPari).smooth_basis(grid_i, X)
                Gcvsave[i] = np.sum(Sm_i["gcv"])
            lambda_s_sm = (
                10 ** loglam[np.argmin(Gcvsave)]
                if lambda_s_ini is None
                else lambda_s_ini
            )
            fdPari = fdpar(fdobj=basis_start, Lfdobj=2, lambda_=lambda_s_sm)
            points = np.asarray(
                BsplineFunc(fdPari).smooth_basis(grid_i, X)["fd"]["coefs"].T
            )
        else:
            basis_start = basis
            ...
            for i in range(N):
                ...
                grid_i = list(
                    np.array(grid)[
                        data["timeindex"][
                            data["curve"] == np.unique(data["curve"])[i]
                        ]
                        - 1
                    ]
                )
                ...
                lambda_s_i_vec[i] = (
                    10 ** loglam[np.argmin(Gcvsave[i, :])]
                    if lambda_s_ini is None
                    else lambda_s_ini
                )
                fdPari = fdpar(
                    fdobj=basis_start, Lfdobj=2, lambda_=lambda_s_i_vec[i]
                )
                points[i, :] = np.asarray(
                    BsplineFunc(fdPari).smooth_basis(grid_i, xi)["fd"]["coefs"]
                )
...
def sasfclust_Mstep(
    ...
    if hard:
        ...
        parameters["pi"] = (
            np.apply_along_axis(np.mean, 0, piigivej) * N + CK
        ) / (N + G * CK)
    ind = np.tile(np.eye(q, dtype=int), (N, 1))
    if perc_rankpapp is not None:
        gsvd = np.linalg.svd(vars_["gprod"] @ ind / N)
        p = np.where(np.cumsum(gsvd[1]) / np.sum(gsvd[1]) >= perc_rankpapp)[0][
            0
        ]
        gsvd[1][p:] = 0
    ...
    if varcon == "diagonal":
        ...
        parameters["Gamma"] = (
            np.eye(q) * np.sum(np.diag(parameters["Gamma"])) / q
        )
    W_star = np.zeros((q * G, q * G))
    for i in range(G):
        W_star[i * q : (i + 1) * q, i * q : (i + 1) * q] = W
    n_i = np.array(
        [np.sum(curve == unique_curve) for unique_curve in np.unique(curve)]
    )
    numden = get_numden(
        data["x"],
        data["curve"],
    ...
    while diff_inter > par_LQA["eps_LQA"] and z_int <= par_LQA["MAX_iter_LQA"]:
        ...
        mu_vec = (
            np.linalg.inv(
                S_den * (1 / parameters["sigma"])
                + lambda_s * 2 * W_star
                + 2 * lambda_l * P_tot.T @ V_l @ P_tot
            )
            @ VY
            * (1 / parameters["sigma"])
        )
...
def get_msdrule(par, sds, comb_list, m1, m2, m3):
    ...
    for lamb_l in lambda_l_g:
        for lamb_s in lambda_s_g:
            indexes = np.where(
                (comb_list[:, 1] == lamb_s) & (comb_list[:, 2] == lamb_l)
            )[0]
    ...
    candidate_idxs_final = np.where(
        par_index_final >= par_index_final[max_idx_final] - lim
    )[0]
    onese_final = (
        candidate_idxs_final[-1]
        if candidate_idxs_final.size > 0
        else max_idx_final
    )
