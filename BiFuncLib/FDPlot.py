...
from BiFuncLib.cc_main_func import (
    template_evaluation,
    medoid_evaluation,
...
class FDPlot:
    ...
    def cc_fdplot(self, data, only_mean=False, aligned=False, warping=False):
        ...
        if shift_alignment == False and (aligned or warping):
            warnings.warn(
                "Warning: no aligned can be performed if results are without alignment"
            )
            aligned = False
            warping = False
        if not aligned and warping:
            warnings.warn(
                "Warning: no warping can be shown if aligned is False"
            )
            warping = False
        if res["Number"] == 0:
        ...
        def plot_single_cluster(df, cluster_num, plot_title, overlay_df=None):
            ...
            cluster_color = col_palette[
                (int(cluster_num) - 1) % len(col_palette)
            ]
        ...
        if aligned:
            ...
            if res["Number"] == 1:
                ...
                clust_cl = data[
                    np.ix_(row_idx, col_idx, np.arange(data.shape[2]))
                ]
                ...
                mat_aligned = np.vstack(
                    [
                        array_aligned[tt, :, :]
                        for tt in range(array_aligned.shape[0])
                    ]
                )
                ...
                temp_cl = np.vstack(
                    [temp_array[tt, :, :] for tt in range(temp_array.shape[0])]
                )
                ...
                fun_aligned = pd.concat(
                    [fun_aligned, fun_aligned_cl], ignore_index=True
                )
                template = pd.concat([template, temp_cl_df], ignore_index=True)
                template_mean = pd.concat(
                    [template_mean, template_mean_cl], ignore_index=True
                )
                warping_cl = pd.DataFrame(res_aligned["x_out"])
                warping_cl["cluster"] = 1
                warping_aligned = pd.concat(
                    [warping_aligned, warping_cl], ignore_index=True
                )
                ...
                for cl in range(res["Number"]):
                    cluster_label = cl + 1
                    row_idx = np.where(
                        np.array(res["RowxNumber"])[:, cl] == True
                    )[0]
                    col_idx = np.where(
                        np.array(res["NumberxCol"])[cl, :] == True
                    )[0]
                    clust_cl = data[
                        np.ix_(row_idx, col_idx, np.arange(data.shape[2]))
                    ]
                    ...
                    temp_cl = np.vstack(
                        [
                            temp_array[tt, :, :]
                            for tt in range(temp_array.shape[0])
                        ]
                    )
                    temp_cl_df = pd.DataFrame(temp_cl)
                    template_mean_cl = pd.DataFrame([temp_cl_df.mean(axis=0)])
                    ...
                    template = pd.concat(
                        [template, temp_cl_df], ignore_index=True
                    )
                    template_mean = pd.concat(
                        [template_mean, template_mean_cl], ignore_index=True
                    )
            ...
            warping_aligned_melt = warping_aligned.melt(
                id_vars=["obs", "cluster"]
            )
            warping_aligned_melt["variable"] = (
                warping_aligned_melt["variable"]
                .astype(str)
                .str.replace("X", "", regex=False)
                .astype(float)
            ...
            if only_mean and warping:
                ...
                for cl in clusters:
                    if alpha == 0 and beta == 0:
                        plot_single_cluster(
                            template_melt, cl, "Representative Functions"
                        )
                    else:
                        plot_single_cluster(
                            template_melt,
                            cl,
                            "Representative Functions with Template",
                            overlay_df=template_mean_melt,
                        )
                    plot_single_cluster(
                        warping_aligned_melt, cl, "Warping Functions"
                    )
                ...
                for cl in clusters:
                    plot_single_cluster(
                        fun_aligned_melt, cl, "Aligned Functions"
                    )
                    plot_single_cluster(
                        warping_aligned_melt, cl, "Warping Functions"
                    )
            ...
            df_fun = pd.DataFrame(
                fun_plot, columns=["X" + str(i) for i in range(1, p + 1)]
            )
            df_fun["obs"] = np.repeat(
                ["ROW " + str(i) for i in range(1, n + 1)], m
            )
            df_fun["var"] = np.tile(
                ["COL " + str(j) for j in range(1, m + 1)], n
            )
            ...
            if res["Number"] == 1:
                ...
                for i in range(res["Number"]):
                    ...
                    xy_matrix += xy
            df_xy = pd.DataFrame(
                xy_matrix, columns=["COL " + str(j) for j in range(1, m + 1)]
            )
            df_xy["obs"] = ["ROW " + str(i) for i in range(1, n + 1)]
            df_xy = pd.melt(
                df_xy, id_vars=["obs"], var_name="variable", value_name="value"
            )
            obs_order = ["ROW " + str(i) for i in range(n, 0, -1)]
            df_xy["obs"] = pd.Categorical(
                df_xy["obs"], categories=obs_order, ordered=True
            )
            df_xy["obj"] = df_xy["obs"].astype(str) + df_xy["variable"].astype(
                str
            )
            df_xy = df_xy[["obj", "value"]].rename(columns={"value": "cluster"})
            data_frame_cl = pd.merge(data_frame, df_xy, on="obj")
            data_frame_cl["variable"] = (
                data_frame_cl["variable"].str.replace("X", "").astype(float)
            )
            template = pd.DataFrame()
            template_mean = pd.DataFrame()
            if res["Number"] == 1:
                ...
                if template_type == "mean":
                    new_fun_cl = template_evaluation(
                        clust_cl, alpha, beta, const_alpha, const_beta
                    )
                elif template_type == "medoid":
                    new_fun_cl = medoid_evaluation(
                        clust_cl, alpha, beta, const_alpha, const_beta
                    )
                temp_cl = np.vstack(
                    [new_fun_cl[tt, :, :] for tt in range(new_fun_cl.shape[0])]
                )
                temp_cl_df = pd.DataFrame(temp_cl)
                template_mean_cl = pd.DataFrame([temp_cl_df.mean(axis=0)])
                temp_cl_df["cluster"] = 1
                ...
                for cl in range(res["Number"]):
                    ...
                    temp_cl = np.vstack(
                        [
                            new_fun_cl[tt, :, :]
                            for tt in range(new_fun_cl.shape[0])
                        ]
                    )
                    temp_cl_df = pd.DataFrame(temp_cl)
                    template_mean_cl = pd.DataFrame([temp_cl_df.mean(axis=0)])
            ...
            if only_mean:
                ...
                for cl in clusters:
                    if cl == 0:
                        continue
                    plot_single_cluster(
                        data_frame_cl[data_frame_cl["cluster"] == cl],
                        cl,
                        "Functional Data",
                    )
    ...
    def fem_fdplot(self, data, fdobj):
        ...
        for i in range(res["K"]):
            ...
            plt.xticks(
                x_ticks, [data["dates"][j] for j in x_ticks], rotation=90
            )
            plt.ylim(0, 1)
        ...
        plt.figure(figsize=(8, 6))
        scatter = plt.scatter(
            np.array(projected_data[:, 0]),
            np.array(projected_data[:, 1]),
            c=res["cls"],
            cmap="tab10",
            s=50,
        )
        ...
        if types == "means":
            ...
            if x["datatype"] == 0:
                ...
                for i in range(x["datatype"]):
                    for l in range(L):
                        obj["coefs"] = np.transpose(
                            x["prms"]["mu"][
                                :,
                                l,
                                (i * x["nbasis"]) : (x["nbasis"] * (i + 1)),
                            ]
                        )
                        plot_fd(obj)
                        plt.show()
            ...
            fig, axs = plt.subplots(1, 2)
            K = x["K"]
            axs[0].bar(range(1, K + 1), x["prms"]["alpha"], color=colors[:K])
            axs[0].set_title(r"$\alpha$")
            axs[0].set_xticks(range(1, K + 1))
            L = x["L"]
            axs[1].bar(range(1, L + 1), x["prms"]["beta"], color=colors[:L])
            axs[1].set_title(r"$\beta$")
    ...
    def sas_cvplot(self):
        ...
        ax1.plot(
            x_vals, CV_i, marker="o", markersize=3, color="red", linestyle="-"
        )
        ax1.errorbar(
            x_vals, CV_i, yerr=sd_i, fmt="none", color="red", capsize=3
        )
        ...
        for jj in range(len(lamb_L)):
            for ii in range(len(lamb_s)):
                indexes = np.where(
                    (comb_list[:, 1] == lamb_s[ii])
                    & (comb_list[:, 2] == lamb_L[jj])
                )[0]
                ...
                lim = (
                    0.5 * abs(np.max(par_index) - np.min(par_index))
                    if (
                        m1_val * sd_index[max_local]
                        > 0.5 * abs(np.max(par_index) - np.min(par_index))
                    )
                    else m1_val * sd_index[max_local]
                )
                candidates = np.where(
                    par_index[: max_local + 1] >= par_index[max_local] - lim
                )[0]
        ...
        for xi, z in zip(x_vals2, zero_vec):
            ax2.text(
                xi,
                np.max(max_vec_nc) + sd_vec_nc.max() * 0.1,
                f"{round(z*100)}",
                ha="center",
                fontsize=8,
            )
        ...
        for ii in range(len(lamb_L)):
            ...
            candidates = np.where(
                par_index_sub
                >= par_index_sub[max_idx_sub]
                - m2_val * sd_index_sub[max_idx_sub]
            )[0]
        ...
        for xi, z in zip(x_vals3, zero_vec2):
            ax3.text(
                xi,
                np.max(max_vec_s) + sd_vec_s.max() * 0.1,
                f"{round(z*100)}",
                ha="center",
                fontsize=8,
            )
