        ...
        if method == "chol":
            if n > nbasis + q or lambda_ > 0:
                ...
                if matwt:
                    ...
                    basisw = (wtvec @ np.ones((1, nbasis + q))) * np.array(
                        basismat
                    )
                ...
                try:
                    ...
                    if Brank < Bmat.shape[0]:
                        print(
                            f"Warning: Matrix of basis function values has rank {Brank} < {Bmat.shape[0]}, ignoring null space"
                        )
                    goodVec = Beig[1][:, BgoodEig]
                    Bmatinv = (
                        goodVec @ np.diag(1 / Beig[0][BgoodEig]) @ goodVec.T
                    )
                else:
            ...
            if n > nbasis or lambda_ > 0:
                if not onewt:
                    if matwt:
                        ...
                        rtwtvec = np.sqrt(wtvec)
                        basismat_aug = (
                            np.tile(rtwtvec, (nbasis, 1)).T * basismat
                        )
                        if len(dimy) < 3:
                ...
                if lambda_ > 0:
                    if penmat is None:
                        penmat = BsplineFunc(
                            basisobj=basisobj, Lfdobj=Lfdobj
                        ).penalty_matrix("fourier")
                    eigvals, eigvecs = eigh(penmat)
                    neiglow = nbasis - nderiv
                    if eigvals[neiglow] <= 0:
                        raise ValueError(
                            "Eigenvalue(NBASIS-NDERIV) of penalty matrix is not positive."
                        )
                    penfac = eigvecs[:, :neiglow] @ np.diag(
                        np.sqrt(eigvals[:neiglow])
                    )
                    basismat_aug = np.r_[
                        basismat_aug, np.sqrt(lambda_) * penfac.T
                    ]
                    if len(dimy) < 3:
                        ...
                        y = np.zeros(
                            (
                                y.shape[0] + nbasis - nderiv,
                                y.shape[1],
                                y.shape[2],
                            )
                        )
                        y[: y0.shape[0], :, :] = y0
                        idx_tail = slice(-(nbasis - nderiv), None)
                        for ivar in range(nvar):
                            y[idx_tail, :, ivar] = 0
                else:
                    penmat = None
                if covariates is not None:
                    ...
                    ind2 = np.arange(nbasis, nbasis + q)
                    basismat_aug = np.c_[
                        basismat_aug, np.zeros((basismat_aug.shape[0], q))
                    ]
                    if not onewt:
                        if matwt:
                            basismat_aug[ind1[:, None], ind2] = (
                                wtfac @ covariates
                            )
                        else:
                        ...
                        basismat_aug[ind1[:, None], ind2] = covariates
                Q, R = qr(basismat_aug, mode="economic")
                if len(dimy) < 3:
                    QTb = Q.T @ y
                    coef = lstsq(R, QTb)[0]
                    if covariates is not None:
                ...
                if n == nbasis + q:
                    ...
                    raise ValueError(
                        f"The number of basis functions = {nbasis + q} exceeds "
                        f"{n} = the number of points to be smoothed."
                    )
        if onewt:
