...
def criteria(loglik, T, prms, n):
    ...
    if model == "DkBk":
        ...
        comp = (
            (K - 1) + K * (K - 1) + (K - 1) * (p - K / 2) + K * (K - 1) // 2 + K
        )
    elif model == "DB":
        comp = (
            (K - 1) + K * (K - 1) + (K - 1) * (p - K / 2) + K * (K - 1) // 2 + 1
        )
    elif model == "AkjBk":
...
def estep(prms, fd, U):
    ...
    for k in range(K):
        ...
        if d == 1:
            for i in range(n):
                QQ[i, k] = (
                    1 / D[k, 0, 0] * np.sum(np.square(projYY[i, :]))
            ...
            for i in range(n):
                QQ[i, k] = (
                    projYY[i, :] @ sY @ projYY[i, :].T
                    + 1 / bk * np.sum(np.square(YY[i, :] - projYY[i, :]))
                    + (p - d) * np.log(bk)
                    + np.log(np.linalg.det(D[k, :d, :d]))
                    - 2 * np.log(prop[k])
                    + p * np.log(2 * np.pi)
                )
    A = -0.5 * QQ
    loglik = np.sum(
        np.log(np.sum(np.exp(A - np.max(A, axis=1, keepdims=True)), axis=1))
        + np.max(A, axis=1)
    )
    for k in range(K):
        T[:, k] = 1 / np.sum(
            np.exp(0.5 * (QQ[:, k].reshape(-1, 1) - QQ)), axis=1
        )
...
def fstep(fd, T, lambda_):
    ...
    U = svd(
        pinv(G.T @ G @ W) @ (G.T @ Ttilde @ Ttilde.T @ G @ W),
        full_matrices=False,
    )[0][:, :d]
    if lambda_ > 0:
        X = G @ U
...
def mstep(fd, U, T, model):
    ...
    for k in range(K):
        ...
        mu[k, :] = (
            np.sum(
                T[:, k].reshape(-1, 1) * np.ones((n, d)) * np.array(X), axis=0
            )
            / nk
        )
        m[k, :] = (
            np.sum(
                T[:, k].reshape(-1, 1) * np.ones((n, p)) * np.array(Y), axis=0
            )
            / nk
        )
