...
def cc_bifunc(
    ...
    if template_type == "medoid" and (alpha != 0 or beta != 0):
        raise ValueError(
            "Error: Medoid template is defined only for alpha and beta equal to 0"
        )
    ...
    if max_iter_align <= 0:
        raise ValueError(
            "Error: max.iter.align must be an integer greater than 0"
        )
    if not isinstance(const_alpha, bool) or not isinstance(const_beta, bool):
        raise ValueError(
            "Error: const_alpha and const_beta must be TRUE or FALSE"
        )
    if delta < 0:
    ...
    for i in range(number):
        ...
        n_clust -= 1
        if (np.sum(erg[0]) == 0 and n_clust == 0) or (
            np.sum(erg[0]) == 0 and i == 0
        ):
            break
    ...
    for d in delta_list:
        ...
        if res_fun["Number"] == 0:
            ...
            for cl in range(res_fun["Number"]):
                ...
                submatrix = data[
                    np.ix_(rows_idx, cols_idx, np.arange(data.shape[0]))
                ]
