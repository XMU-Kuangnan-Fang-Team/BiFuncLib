...
def pf_bifunc(
            ...
            temp_df = pd.DataFrame(
                {
                    ...
                    "y": current_measurement_df[
                        current_measurement_df.columns[i]
                    ],
                }
            )
            reformed_dataframe = pd.concat(
    ...
    if opt == False:
        ...
        # ADMM algorithm
        inv_UTY_result = inv_uty_cal(
            spline_list, Y_list, D_d, n, q, p, gamma1, theta
        )
        Beta_ini = beta_ini_cal(spline_list, Y_list, D_d, n, q, p, gamma1)
        result = biclustr_admm(
            inv_UTY_result,
            data_list,
            Y_list,
            D_d,
        ...
        # Clustering
        Ad_final_sam = AuxFunc(
            n=n, V=result["V1"]
        ).create_adjacency()  # Row clustering membership
        G_final_sam = nx.from_numpy_array(Ad_final_sam)
        cls_final_sam = list(nx.connected_components(G_final_sam))
        Ad_final_fea = AuxFunc(
            n=q, V=result["V2"]
        ).create_adjacency()  # Column clustering membership
        ...
        for l in range(len(gamma1)):
            Beta_ini = beta_ini_cal(
                spline_list, Y_list, D_d, n, q, p, gamma1[l]
            )
            Beta_mat = np.reshape(Beta_ini, (p * q, n), order="F")
            for i in range(n):
                for j in range(q):
                    y_hat = np.dot(
                        spline_list[i][j], Beta_mat[(j * p) : (j + 1) * p, i]
                    )
                    ...
                    df_1 = np.trace(
                        np.dot(
                            spline_list[i][j],
                            solve(
                                np.dot(spline_list[i][j].T, spline_list[i][j])
                                + gamma1[l] * D_d,
                                spline_list[i][j].T,
                            ),
                        )
                    ...
                    bic_mat[i, j] = (
                        np.log(np.sum((Y_list[i][j] - y_hat) ** 2) / n_1)
                        + np.log(n_1) * df_1 / n_1
                    )
            bic_value_1[l] = np.sum(bic_mat)
        opt_gamma1 = gamma1[np.argmin(bic_value_1)]
        inv_UTY_result = inv_uty_cal(
            spline_list, Y_list, D_d, n, q, p, gamma1=opt_gamma1, theta=theta
        )
        Beta_ini = beta_ini_cal(
            spline_list, Y_list, D_d, n, q, p, gamma1=opt_gamma1
        )
        ...
        for l in range(len(gamma2)):
            ...
            cls_final_fea = list(nx.connected_components(G_final_fea))
            vstacked_data = [
                np.vstack(sublist).flatten()[:, np.newaxis]
                for sublist in result["Beta"]
            ]
            Beta_mat = np.column_stack(vstacked_data)
            Beta_list = [
                [Beta_mat[(j * p) : (j + 1) * p, i] for j in range(q)]
                for i in range(n)
            ]
            Y_hat_list = [
                np.concatenate(
                    [
                        np.dot(spline_list[i][j], Beta_list[i][j])
                        for j in range(q)
                    ]
                )
                for i in range(n)
